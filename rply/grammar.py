...
class Grammar:
        ...
        if precedence is None:
            ...
            try:
                ...
                raise ParserGeneratorError(
                    f"Precedence {precedence!r} doesn't exist"
                )
    ...
    def set_precedence(self, term, assoc, level):
        if term in self.precedence:
            raise ParserGeneratorError(
                f'Precedence already specified for {term}'
            )
    ...
    def unused_terminals(self):
        return [
            t for t, prods in self.terminals.items()
...
class Production:
    ...
    def __repr__(self):
        production = ' '.join(self.prod)
        return f'Production({self.name} -> {production})'
...
class LRItem:
    ...
    def __repr__(self):
        production = ' '.join(self.prod)
        return f'LRItem({self.name} -> {production})'
